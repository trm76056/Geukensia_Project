...
GENOME.class <- set.populations(GENOME.class,list(
    ...
    c("MA12-30","MA15-30","MA16-20","MA17-20","MA18-20","MA20-20","MAC1","MAC2","MAC3","MAC5")))
...
#fuliD_ga <- ggplot(fuliD.df) +aes(x = pop.1) +geom_histogram(bins = 30L, fill = "#000000") +
#  labs(x = "Fu and Li's D*",y = "Number")+
#  theme_minimal(base_size = 22)
#fuliD_ma <- ggplot(fuliD.df) +aes(x = pop.2) +geom_histogram(bins = 30L, fill = "#000000") +
#  labs(x = "Fu and Li's D*",y = "Number") +
#  theme_minimal(base_size = 22)
...
phiSTplot <- ggplot(as.data.frame(phi_st)) + aes(x = `Phi_ST`) + geom_histogram(bins = 30L, fill = "#000000") +
  labs(x = expression(paste(Phi)[ST]),y="Number") +
  theme_minimal(base_size = 22)
pi_persiteGAplot <- ggplot(pi_persite.df) +aes(x = pop.1) +geom_histogram(bins = 30L, fill = "#000000") +
...
pi_persiteMAplot <- ggplot(pi_persite.df) +aes(x = pop.2) +geom_histogram(bins = 30L, fill = "#000000") +
  labs(x = expression(paste(Pi," per site")),y = "Number") +
  theme_minimal() + theme(axis.text=element_text(size=10),axis.title=element_text(size=22)) + scale_y_continuous(trans=scales::pseudo_log_trans(base = 10))
...
#within_plots <- plot_grid(pi_persiteGAplot,pi_persiteMAplot,fuliD_ga,fuliD_ma,labels="AUTO")
#save_plot("WithinPopDiversityPlots_noMA13-30.png",within_plots,ncol=2,nrow=2)
#use tajima's D instead of fu and li's D
within_plots <- plot_grid(pi_persiteGAplot,pi_persiteMAplot,tajd_ga,tajd_ma,labels="AUTO")
save_plot("WithinPopDiversityPlots_noMA13-30.png",within_plots,ncol=2,nrow=2)

#create figure for between population diversity stats
#between_plots <- plot_grid(netPlot,snnPlot,phiSTplot,labels="AUTO")
#save_plot("BetweenPopDiversityPlots_noMA13-30.png",between_plots,ncol=2,nrow=2)
#don't use phiST
between_plots <- plot_grid(netPlot,snnPlot,labels="AUTO")
save_plot("BetweenPopDiversityPlots_noMA13-30.png",between_plots,ncol=2,nrow=1)
...
#this part is to pull out the orthologs of interest based on criteria
#pi per site > 0.01 and Tajimas D > 0.05 AND dA > 0.01 or p < 0.05 for Snn
